...
{
    Task<IReadOnlyList<ParkingAreaDto>> GetAreasAsync(
        int? venueId = null,
        CancellationToken cancellationToken = default);

    Task<ParkingAreaDto> GetAreaByIdAsync(
        int areaId,
        CancellationToken cancellationToken = default);

    Task<ParkingAreaDto> CreateAreaAsync(
        CreateParkingAreaDto dto,
        CancellationToken cancellationToken = default);

    Task<ParkingAreaDto> UpdateAreaAsync(
        int areaId,
    ...
    Task<ParkingLayoutDto> GetEventLayoutAsync(
        int eventId,
        int? actorOrganizerId,
        string? actorRole,
        CancellationToken cancellationToken = default);

    Task<IReadOnlyList<ParkingSlotDto>> GetEventParkingSlotsAsync(
        int eventId,
        int? actorOrganizerId,
        string? actorRole,
        CancellationToken cancellationToken = default);

    Task<EventParkingAllocationDto> AllocateAreaAsync(
        int eventId,
